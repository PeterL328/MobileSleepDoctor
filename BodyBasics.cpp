...
static std::stringstream docName;

static bool entered_before = false;
static clock_t body_enter;
static clock_t body_exit;
...
void CBodyBasics::ProcessBody(INT64 nTime, int nBodyCount, IBody** ppBodies)
{
	...
	if (total_bodies > 0 && entered_before == false) {
		entered_before = true;
		body_enter = clock();
	} else if (total_bodies == 0 && entered_before == true) {
		docName.str(std::string());
		docName << "C:\\Python27\\python C:\\Python27\\predict.py " << (clock() - body_enter)/1000 
			<< " D:\\Applications\\kinect\\BodyBasics-D2D\\data.csv";
		std::string arg = docName.str();
    ...
    if (m_hWnd)
    {
		...
		StringCchPrintf(szStatusMessage, _countof(szStatusMessage), L" FPS = %0.2f    Time = %I64d        \tTest = ", fps, (nTime - m_nStartTime));
...
{
	//static time_t time_start = time(0);
	//rec < 100 && rec % 10 == 0
